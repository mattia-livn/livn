...
class CommuneFilesAnalyzer {
  ...
  async analyzeStructure() {
    ...
    const files = fs.readdirSync(statementsDir).filter(f => f.endsWith('.ts')).slice(0, 3);
    
    for (const file of files) {
      await this.analyzeFile(path.join(statementsDir, file));
    }
  }
  ...
  private findFirstMalformedObject(lines: string[]): number {
  ...
  private findValidObjects(lines: string[]): Array<{startLine: number, condition: string, ratePercent: number}> {
    const validObjects = [];
    let inObject = false;
    let currentObject = '';
    let startLine = -1;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inObject) {
        ...
        if (braceLevel <= 0 && line.includes('}')) {
          ...
          if (validObjects.length >= 3) break;
        }
      }
    }
  ...
  private extractCondition(objectStr: string): string | null {
    const match = objectStr.match(/condition:\s*["']([^"']+)["']/);
    return match ? match[1] : null;
  }
  
  private extractRatePercent(objectStr: string): number | null {
