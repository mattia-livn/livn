...
function loadCredentials() {
  ...
  return {
    baseUrl: env.OPENAPI_URL || env.OPENAPI_SANDBOX_URL,
    apiKey: env.OPENAPI_KEY || env.OPENAPI_SANDBOX_KEY,
    token: env.OPENAPI_TOKEN || env.OPENAPI_SANDBOX_TOKEN
  };
}
...
async function testCatastoService() {
  ...
  try {
    ...
    const emptyService = new CatastoService(configEmpty);
    
    // Override per ritornare risultati vuoti
    mockEmptyResponses();

    const { fabbricati: emptyFabbricati, terreni: emptyTerreni } = 
      await emptyService.raccogliDatiCompleti('00000000000');
