...
interface MockUserAnswers {
  abitazione_principale?: string;
  pertinenze?: Record<string, number[]>;
  condizioni_immobili?: Record<string, string>;
  domande_specifiche_comune?: Record<string, any>;
}

class MockLivnAPIServer {
  private communeConditionsService = new CommuneConditionsService();

  async generateQuestions(extractedData: MockExtractedData, userAnswers: MockUserAnswers): Promise<any[]> {
    ...
    if (!userAnswers.abitazione_principale) {
      ...
      if (abitazioni.length > 0) {
        questions.push({
          ...
          options: [
            ...abitazioni.map((hab: any, index: number) => ({
    ...
    if (comunePrincipale && userAnswers.condizioni_immobili && !userAnswers.domande_specifiche_comune) {
      ...
      try {
        const domandeSpecifiche = await this.communeConditionsService.generateCommuneSpecificQuestions(
          extractedData,
          comunePrincipale
        );
  ...
  async calculateIMU(extractedData: MockExtractedData, userAnswers: MockUserAnswers) {
...
async function testIntegrationEndToEnd() {
  ...
  const extractedDataMilano: MockExtractedData = {
    fabbricati: [
      {
        categoria: 'A/2',
        comune: 'Milano',
  ...
  let userAnswers: MockUserAnswers = {};
  ...
  console.log('Prima domanda:', questions[0]?.title);
  ...
  if (questions.length > 0) {
    console.log('Domanda specifica:', questions[0]?.title);
  }
  ...
  risultato.details.forEach((detail: any, i: number) => {
  ...
  const verificaPersonalizzazione = risultato.details.some((detail: any) => 
    !detail.tipo.includes('fallback') && 
