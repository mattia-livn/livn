...
exports.TorinIMUQuestionGenerator = void 0;
exports.generateQuestionsForProperties = generateQuestionsForProperties;
const torinoTO2025_1 = require("./torinoTO2025");
class TorinIMUQuestionGenerator {
    constructor() {
        this.rates = torinoTO2025_1.imuRatesTorinoTO2025;
    }
    ...
    generateOwnerQuestions(property, potentialRates) {
        const questions = [];
        const applicableRates = [];
        // Controllo per agevolazioni anziani/disabili
        const elderlyDisabledRates = potentialRates.filter(r => r.conditions.some(c => c.predicate.includes('ownerIsElderlyOrDisabled')));
        ...
        const agriculturalRates = potentialRates.filter(r => r.conditions.some(c => c.predicate.includes('ownerIsAgriculturalEntrepreneur')));
    ...
    generateUsageQuestions(property, potentialRates) {
        ...
        const applicableRates = [];
        // Uso agricolo
        const agriculturalUseRates = potentialRates.filter(r => r.conditions.some(c => c.predicate.includes('isAgriculturalUse')));
        ...
        const atecoRates = potentialRates.filter(r => r.conditions.some(c => c.predicate.includes('atecoCode')));
    ...
    generateContractQuestions(property, potentialRates) {
        ...
        const rentalRates = potentialRates.filter(r => r.conditions.some(c => c.predicate.includes('contractType')) ||
    ...
    extractAtecoCodesFromRates(rates) {
        const codes = [];
        rates.forEach(rate => {
...
exports.TorinIMUQuestionGenerator = TorinIMUQuestionGenerator;
// Funzione di utilità per generare tutte le domande per una lista di immobili
function generateQuestionsForProperties(properties) {
