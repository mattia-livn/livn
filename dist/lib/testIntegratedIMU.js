...
exports.quickIMUTest = exports.runAllIntegratedTests = exports.runIntegratedIMUTest = exports.loadTestData = void 0;
const fs_1 = require("fs");
const path_1 = require("path");
const imuCalculator_1 = require("./imuCalculator");
const testScenarios = [
    {
...
const loadTestData = () => {
    try {
        const dataPath = (0, path_1.join)(process.cwd(), 'data', 'test_data.json');
...
const runIntegratedIMUTest = async (scenario) => {
    console.log(`\n🏗️ === ${scenario.name} ===`);
    console.log(`📄 ${scenario.description}`);
    console.log(`🏛️ Aliquota comunale: ${scenario.municipalRate}‰`);
    try {
        // Carica i dati di test
        const cadastralData = (0, exports.loadTestData)();
        if (!cadastralData) {
        ...
        const result = await (0, imuCalculator_1.calculateIMUWithAI)(cadastralData, scenario.municipalRate);
        const endTime = Date.now();
        console.log(`⏱️ Calcolo completato in ${endTime - startTime}ms`);
        // Mostra i risultati
        (0, imuCalculator_1.displayIMUResults)(result);
...
exports.runIntegratedIMUTest = runIntegratedIMUTest;
const runAllIntegratedTests = async () => {
    console.log('🚀 === SUITE TEST INTEGRATI IMU ===');
    console.log(`📊 Numero di scenari: ${testScenarios.length}`);
    let successCount = 0;
    let failCount = 0;
    for (const scenario of testScenarios) {
        const success = await (0, exports.runIntegratedIMUTest)(scenario);
        if (success) {
            successCount++;
        }
        else {
...
exports.runAllIntegratedTests = runAllIntegratedTests;
// Test rapido per sviluppo
const quickIMUTest = async () => {
    ...
    return await (0, exports.runIntegratedIMUTest)(testScenarios[0]);
};
exports.quickIMUTest = quickIMUTest;
//# sourceMappingURL=testIntegratedIMU.js.map