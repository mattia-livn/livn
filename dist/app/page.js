...
function Home() {
    ...
    const [reportLoading, setReportLoading] = (0, react_1.useState)(false);
    const [deliberaLoading, setDeliberaLoading] = (0, react_1.useState)(false);
    const [result, setResult] = (0, react_1.useState)(null);
    const [delibereFiles, setDelibereFiles] = (0, react_1.useState)([]);
    const runTest = async () => {
        setLoading(true);
        setResult(null);
        try {
    ...
    const analyzeDelibera = async (fileName) => {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
          ...
          <div className="flex gap-4 flex-wrap">
            <button onClick={runTest} disabled={loading || reportLoading || deliberaLoading} className="bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white px-6 py-3 rounded-lg font-medium transition-colors">
              {loading ? '🔄 Calcolo in corso...' : '🚀 Avvia Test IMU'}
            </button>
            
            <button onClick={downloadReport} disabled={loading || reportLoading || deliberaLoading} className="bg-green-600 hover:bg-green-700 disabled:bg-green-300 text-white px-6 py-3 rounded-lg font-medium transition-colors">
        ...
        <div className="bg-purple-50 rounded-lg shadow-lg p-6 mb-8">
          ...
          <div className="flex gap-4 flex-wrap mb-4">
            <button onClick={loadDelibereFiles} disabled={loading || reportLoading || deliberaLoading} className="bg-purple-600 hover:bg-purple-700 disabled:bg-purple-300 text-white px-6 py-3 rounded-lg font-medium transition-colors">
              📂 Carica Lista Delibere
            </button>
          </div>

          {delibereFiles.length > 0 && (<div className="mt-4">
              <h3 className="text-lg font-medium mb-3">📄 File Disponibili:</h3>
              <div className="grid gap-2">
                {delibereFiles.map((fileName, index) => (<button key={index} onClick={() => analyzeDelibera(fileName)} disabled={loading || reportLoading || deliberaLoading} className="bg-gray-100 hover:bg-gray-200 disabled:bg-gray-50 text-left px-4 py-2 rounded-lg text-sm transition-colors">
        ...
        {result && (<div className="bg-white rounded-lg shadow-lg p-6">
            ...
            {result.success && result.filename && (<div className="mt-4 p-4 bg-green-50 border border-green-200 rounded">
            ...
            {result.success && result.analysis && (<div className="mt-4 p-4 bg-purple-50 border border-purple-200 rounded">
            ...
            {result.success && !result.filename && !result.analysis && (<div className="mt-4 p-4 bg-green-50 border border-green-200 rounded">
                <p className="text-green-800">
            ...
            {!result.success && (<div className="mt-4 p-4 bg-red-50 border border-red-200 rounded">
                <p className="text-red-800">
                  ❌ Si è verificato un errore durante l'elaborazione.
                  Verifica che le chiavi API siano configurate correttamente.
                </p>
                {result.error && (<p className="text-red-600 text-sm mt-2">
                    Errore: {result.error}
                  </p>)}
              </div>)}
          </div>)}
