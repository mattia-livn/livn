...
class VisureParser {
     ...
     */
    async parseVisura(filePath, fileName) {
        const result = {
            success: false,
            errors: [],
            fabbricati: [],
        ...
        try {
            ...
            result.errors.push(`Errore durante il parsing: ${error.message}`);
        }
        return result;
    }
    /**
     ...
     */
    async parsePDFVisura(filePath, result) {
        ...
        try {
            ...
            result.errors.push(`Errore parsing PDF: ${error.message}`);
        }
    }
    /**
     * Parsa visura in formato testo
     */
    async parseTextVisura(filePath, result) {
        try {
            const content = fs.readFileSync(filePath, 'utf8');
            this.parseTextContent(content, result);
        }
        catch (error) {
            result.errors.push(`Errore lettura file testo: ${error.message}`);
        }
    }
    /**
     * Parsa visura in formato CSV
     */
    async parseCSVVisura(filePath, result) {
        try {
            const content = fs.readFileSync(filePath, 'utf8');
            this.parseCSVContent(content, result);
        }
        catch (error) {
            result.errors.push(`Errore parsing CSV: ${error.message}`);
        }
    }
    /**
     * Estrae dati da contenuto testuale di visura
     */
    parseTextContent(content, result) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (matchFabbricato && proprietarioCorrente) {
                ...
                const fabbricato = {
                    ...
                    piano: '',
                    interno: '',
                    proprietario: Object.assign({}, proprietarioCorrente),
     ...
     */
    parseCSVContent(content, result) {
        var _a, _b, _c, _d;
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            try {
                if (((_a = row.tipo) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === 'fabbricato' || ((_b = row.tipo) === null || _b === void 0 ? void 0 : _b.toLowerCase()) === 'f') {
                    const fabbricato = {
                        comune: row.comune,
                        ...
                        piano: row.piano || '',
                        interno: row.interno || '',
                        proprietario: Object.assign({}, proprietarioDefault),
                        dataAggiornamento: new Date(),
                        idImmobile: `${row.foglio}-${row.particella}-${row.subalterno || '0'}`
                    };
                    result.fabbricati.push(fabbricato);
                }
                else if (((_c = row.tipo) === null || _c === void 0 ? void 0 : _c.toLowerCase()) === 'terreno' || ((_d = row.tipo) === null || _d === void 0 ? void 0 : _d.toLowerCase()) === 't') {
                    const terreno = {
                        comune: row.comune,
                        provincia: row.provincia,
                        codiceCatastale: row.codice_catastale || '',
                ...
                result.errors.push(`Riga ${i + 1}: errore parsing dati - ${error.message}`);
            }
        }
    }
    /**
    ...
    validateExtractedData(data) {
        ...
        data.fabbricati.forEach((fab, idx) => {
            if (!fab.comune)
                errors.push(`Fabbricato ${idx + 1}: comune mancante`);
            if (!fab.provincia)
                errors.push(`Fabbricato ${idx + 1}: provincia mancante`);
            if (!fab.foglio || fab.foglio <= 0)
                errors.push(`Fabbricato ${idx + 1}: foglio non valido`);
            if (!fab.particella || fab.particella <= 0)
                errors.push(`Fabbricato ${idx + 1}: particella non valida`);
            if (!fab.categoria)
                errors.push(`Fabbricato ${idx + 1}: categoria mancante`);
            if (!fab.rendita || fab.rendita <= 0)
                errors.push(`Fabbricato ${idx + 1}: rendita non valida`);
        });
        // Validazione terreni
        data.terreni.forEach((ter, idx) => {
            if (!ter.comune)
                errors.push(`Terreno ${idx + 1}: comune mancante`);
            if (!ter.provincia)
                errors.push(`Terreno ${idx + 1}: provincia mancante`);
            if (!ter.foglio || ter.foglio <= 0)
                errors.push(`Terreno ${idx + 1}: foglio non valido`);
            if (!ter.particella || ter.particella <= 0)
                errors.push(`Terreno ${idx + 1}: particella non valida`);
            if (!ter.qualita)
                errors.push(`Terreno ${idx + 1}: qualità mancante`);
            if (!ter.redditoDominicale || ter.redditoDominicale <= 0)
                errors.push(`Terreno ${idx + 1}: reddito dominicale non valido`);
        });
        return errors;
    }
}
exports.VisureParser = VisureParser;
//# sourceMappingURL=visure-parser.js.map