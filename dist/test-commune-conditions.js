...
async function testCommuneConditionsService() {
    ...
    const service = new commune_conditions_service_1.CommuneConditionsService();
    ...
    try {
        const result = await service.findBestCondition(testFabbricato, testUserAnswers, 0, 'abanoterme');
        ...
        console.log(`   Condizione matched: ${((_a = result.conditionMatched) === null || _a === void 0 ? void 0 : _a.condition) || 'Nessuna'}`);
    }
    catch (error) {
    ...
    try {
        const result = await service.findBestCondition(testFabbricato, testUserAnswers, 0, 'abbadialariana');
    ...
    try {
        const result = await service.findBestCondition(testFabbricato, testUserAnswers, 0, 'abbasanta');
    ...
    try {
        const questions = await service.generateCommuneSpecificQuestions(extractedData, 'abanoterme');
...
//# sourceMappingURL=test-commune-conditions.js.map